...
Bernouilli:
\begin{math}
  \text{Y = 1 if succsses otherwise 0 with probability p and (1-p)}
    \expect{Y} = p,\ 
    \expect{Y^2} = p\ (1^2 = 1, 0^2 = 0),\ 
    Var(Y) = \expect{Y}-(\expect{Y})^2 = p - p^2 = p(1-p)\
    \sum_{i=1}^n\left[Y_i \right] = X \sim Bin(n,p) \Rightarrow 
    \expect{X} = \expect{\sum_{i=1}^n\left[Y_i \right]} = \expect{Y_1}+...+\expect{Y_n} = np,\
    Var(X) = Var(\sum_{i=1}^n\left[Var(Y_i) \right])
           = \sum_{i=1}^n\left[p(1-p) \right]
           = np(1-p)
\end{math} \\[2pt]
\newpage
