...
\section{Sequences}
\begin{math}
  x^0 + cx^1 + c^2x^2 + \dotsb = \sum\limits_{k = 0}^{\infty} (cx)^k = \frac{1}{1 - cx}
\end{math} \\[2pt]
\begin{math}
...
\end{math} \\[2pt]
\begin{math}
  EXY = \sum_x \left[ \sum_y\left[xy\cdot f_{xy}(x,y)\right] \right ]
      = \text{independent} 
      = \sum_x \left[\sum_y{\left[ xy\cdot f_x(x)f_y(y)\right] }\right] 
      = \sum_x \left[{xf_x(x)}\sum_y\left[{yf_y(y)\right ]}\right ]
      = EXEY
\end{math} \\[2pt]
\begin{math}
  E[X+Y] = \sum_r\left[ r\cdot P(X+Y=r)\right]
         = \sum_{k,j}\left[ (x_{j}+y_{k})P(X=x_{j},Y=y_{j})\right ]
         = \sum_{k,j}\left[ x_{j}\cdot P(X=x_{j},Y=y_{j})\right ] + 
           \sum_{k,j}\left[ y_{k}\cdot P(X=x_{j},Y=y_{j})\right ] 
         = \sum_j\left[x_{j}\sum_k \left [ P(X=x_{j},Y=y_{j})\right ] \right ] + 
           \sum_k\left[ y_{k}\sum_y\left[ P(X=x_{j},Y=y_{j})\right ]\right ]
...
\begin{math}
  Var(X+Y) = E((X+Y)^2) - (E(X+Y))^2 
           ...
           = EX^2-(EX)^2 + EY -(EY)^2 + 2E(XY) - 2EXEY 
           = Var(X)+Var(Y)+2Cov(X,Y)
           = \text{if X,Y independent}=Var(X)+Var(Y)
\end{math} \\[2pt]
% (end)
  %
\newpage
