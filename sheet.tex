...
Chance of absorbing in state a, starting from j: 0 if j absorbing otherwise:
\begin{math}
  q_j = P(A|x_0 = j)
      = \frac{P(A \cap (x_0 = k))}{P(x_0 = j)}
      = \frac{\sum_{k}{P(A \cap (x_0 = j) \cap (x_1 = k))}}{P(x_0 = j)}
      = \sum_k \left[ P(A|x_0 = j) \cdot P(x_1 = k | x_0 = j) \right]
...
\begin{math}
  \expect{h(X)} = \sum_k \left[ k \cdot f_{h(x)}(x) \right]
                = \sum_h \left[ k \cdot P(h(x) = k) \right]
